...
const mediaObserver=new MutationObserver(()=>{document.querySelectorAll('#detail .grid a').forEach(a=>{let path=new URL(a.href).searchParams.get('path')||'';let node=a.querySelector('img');let isVideo=path.toLowerCase().endsWith('.mp4')||path.toLowerCase().endsWith('.mov');if(isVideo&&node&&!a.querySelector('video')){let video=document.createElement('video');video.controls=true;video.preload='metadata';video.muted=true;video.setAttribute('aria-label','Video preview');video.src=a.href;node.replaceWith(video)}if(node&&!node.dataset.errorBound){node.dataset.errorBound='1';node.alt='Preview of '+path;node.onerror=()=>{node.replaceWith(Object.assign(document.createElement('span'),{textContent:'Preview unavailable',className:'muted'}))}}})});
...
HTML_PAGE = (HTML_PAGE.replace("</style>", ".detail{height:100vh;overflow:auto;box-sizing:border-box;position:sticky;top:0}@media(max-width:700px){.detail{position:static;height:auto;overflow:visible}}" + chr(10) + "</style>")
             .replace("join('\n')", "join('\\n')")
